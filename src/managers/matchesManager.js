...
export const fetchMatches = R.pipeP(
  ...
  R.applySpec({
    data: R.pipe(
      ...
      R.map(
        R.applySpec({
          ...
          firstImg: item =>
            R.path(['competitors', '0', 'images', 'logo', 'S1'], item) ===
            undefined
              ? R.path(
                  ['competitors', '0', 'images', 'logo', 'T1', 'url'],
                  item,
                )
              : R.path(
                  ['competitors', '0', 'images', 'logo', 'S1', 'url'],
                  item,
                ),

          secondImg: item =>
            R.path(['competitors', '1', 'images', 'logo', 'S1'], item) ===
            undefined
              ? R.path(
                  ['competitors', '1', 'images', 'logo', 'T1', 'url'],
                  item,
                )
              : R.path(
                  ['competitors', '1', 'images', 'logo', 'S1', 'url'],
                  item,
                ),
    ...
    meta: R.pipe(
      R.path(['response', 'meta']),
      R.converge((limit, count) => Number(limit) === count, [
        R.prop('limit'),
        R.prop('count'),
      ]),
    ),
